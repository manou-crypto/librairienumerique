...
interface LigneInventaire {
  ...
  ecart: number | null;
  statut: 'en_attente' | 'compte' | 'ecart';
}
...
function InventairePage() {
  ...
  const filteredLignes = lignes.filter(l =>
    l.produit.toLowerCase().includes(search.toLowerCase()) || l.ref.toLowerCase().includes(search.toLowerCase())
  );
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-border bg-muted/50">
                    ...
                    <th className="text-left px-5 py-3 text-xs font-semibold text-muted-foreground">Responsable</th>
                    <th className="text-right px-5 py-3 text-xs font-semibold text-muted-foreground">Progression</th>
                    <th className="text-right px-5 py-3 text-xs font-semibold text-muted-foreground">Écarts</th>
                  ...
                  {INVENTAIRES.map((inv, idx) => {
                    ...
                    return (
                      <tr key={inv.id} className={`border-b border-border table-row-hover ${idx % 2 === 0 ? '' : 'bg-muted/20'}`}>
                        ...
                        <td className="px-5 py-3 text-right">
                          <span className={`text-xs font-semibold ${inv.ecarts > 0 ? 'text-negative' : 'text-positive'}`}>
                            {inv.ecarts > 0 ? `−${inv.ecarts}` : '0'}
                          </span>
                        </td>
          ...
          <div className="space-y-4">
            ...
            <div className="card-base p-5">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h3 className="text-sm font-bold text-foreground">INV-2026-08 — En cours</h3>
                  <p className="text-xs text-muted-foreground">Responsable : Ahmed Mansouri · Démarré le 01/08/2026</p>
                </div>
              ...
              <div className="flex items-center gap-6 mt-3 text-xs text-muted-foreground">
                <span><span className="font-semibold text-foreground">{lignes.filter(l => l.statut !== 'en_attente').length}</span> comptés</span>
                <span><span className="font-semibold text-foreground">{lignes.filter(l => l.statut === 'en_attente').length}</span> en attente</span>
                <span><span className="font-semibold text-negative">{lignes.filter(l => l.statut === 'ecart').length}</span> écarts</span>
              </div>
            </div>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-border bg-muted/50">
                      ...
                      <th className="text-right px-5 py-3 text-xs font-semibold text-muted-foreground">Stock système</th>
                      <th className="text-right px-5 py-3 text-xs font-semibold text-muted-foreground">Stock compté</th>
                    ...
                    {filteredLignes.map((ligne, idx) => (
                      <tr key={ligne.id} className={`border-b border-border table-row-hover ${idx % 2 === 0 ? '' : 'bg-muted/20'}`}>
                        ...
                        <td className="px-5 py-3 text-right tabular-nums font-semibold">
                          {ligne.ecart !== null ? (
                            <span className={ligne.ecart < 0 ? 'text-negative' : ligne.ecart > 0 ? 'text-positive' : 'text-muted-foreground'}>
                        ...
                        <td className="px-5 py-3 text-center">
                          {ligne.statut === 'en_attente' && <span className="badge-hidden">En attente</span>}
                          {ligne.statut === 'compte' && <span className="badge-active">Compté</span>}
                          {ligne.statut === 'ecart' && <span className="badge-rupture">Écart</span>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
