    ...
    <AppLayout currentPath="/stock">
      ...
      <div className="px-6 py-6 max-w-screen-2xl mx-auto space-y-6">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="kpi-card-info"><p className="text-xs text-muted-foreground mb-1">Valeur totale du stock</p><p className="text-xl font-bold text-foreground tabular-nums">{totalValeur.toLocaleString('fr-DZ')} DA</p><p className="text-xs text-muted-foreground mt-1">{STOCK_DATA.length} références</p></div>
              ...
              <tbody>
                {filtered.map((item, idx) => (
                  <tr key={item.id} className={`border-b border-border table-row-hover ${idx % 2 === 0 ? '' : 'bg-muted/20'}`}>
                    ...
                    <td className="px-5 py-3 text-right tabular-nums font-semibold text-foreground">{item.valeurStock.toLocaleString('fr-DZ')} DA</td>
