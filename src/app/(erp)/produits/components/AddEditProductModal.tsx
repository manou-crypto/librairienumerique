        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div><label className="block text-xs font-semibold text-foreground mb-1.5" htmlFor="prixAchat">Prix d&apos;achat (€) <span className="text-negative">*</span></label><input id="prixAchat" type="number" step="0.01" min="0" {...register('prixAchat', { required: 'Obligatoire.', min: { value: 0, message: 'Doit être positif.' }, valueAsNumber: true })} className="input-field tabular-nums" />{errors.prixAchat && <p className="flex items-center gap-1 text-xs text-negative mt-1.5"><AlertCircle size={12} />{errors.prixAchat.message}</p>}</div>
              <div><label className="block text-xs font-semibold text-foreground mb-1.5" htmlFor="prixVente">Prix de vente (€) <span className="text-negative">*</span></label><input id="prixVente" type="number" step="0.01" min="0" {...register('prixVente', { required: 'Obligatoire.', min: { value: 0.01, message: 'Doit être supérieur à 0.' }, valueAsNumber: true })} className="input-field tabular-nums" />{errors.prixVente && <p className="flex items-center gap-1 text-xs text-negative mt-1.5"><AlertCircle size={12} />{errors.prixVente.message}</p>}</div>
              <div><label className="block text-xs font-semibold text-foreground mb-1.5">Marge brute</label><div className="input-field bg-muted/50 flex items-center gap-2 cursor-not-allowed"><span className={`text-sm font-bold tabular-nums ${Number(marge) >= 40 ? 'text-green-600' : Number(marge) >= 25 ? 'text-blue-600' : 'text-amber-600'}`}>{marge}%</span><span className="text-xs text-muted-foreground">(+{benefice} €/unité)</span></div></div>
