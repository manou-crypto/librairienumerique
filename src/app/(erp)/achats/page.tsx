    ...
    <AppLayout currentPath="/achats">
      ...
      <div className="px-6 py-6 max-w-screen-2xl mx-auto space-y-6">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="kpi-card-info"><p className="text-xs text-muted-foreground mb-1">Total achats (mois)</p><p className="text-xl font-bold text-foreground tabular-nums">{totalMontant.toLocaleString('fr-DZ')} DA</p><p className="text-xs text-muted-foreground mt-1">{ACHATS.filter(a => a.statut === 'recu').length} commandes reçues</p></div>
              ...
              <tbody>
                {filtered.map((achat, idx) => (
                  <tr key={achat.id} className={`border-b border-border table-row-hover ${idx % 2 === 0 ? '' : 'bg-muted/20'}`}>
                    ...
                    <td className="px-5 py-3 text-right tabular-nums font-semibold text-foreground">{achat.montantHT.toLocaleString('fr-DZ')} DA</td>
