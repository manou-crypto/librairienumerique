    ...
    <AppLayout currentPath="/ventes">
      ...
      <div className="px-6 py-6 max-w-screen-2xl mx-auto space-y-6">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="kpi-card-info"><p className="text-xs text-muted-foreground mb-1">CA aujourd'hui</p><p className="text-xl font-bold text-foreground tabular-nums">{totalJour.toLocaleString('fr-DZ')} DA</p><p className="text-xs text-muted-foreground mt-1">{VENTES.filter(v => v.date === '2026-08-03').length} ventes</p></div>
          <div className="kpi-card-positive"><p className="text-xs text-muted-foreground mb-1">CA total (période)</p><p className="text-xl font-bold text-foreground tabular-nums">{totalMois.toLocaleString('fr-DZ')} DA</p><p className="text-xs text-muted-foreground mt-1">{VENTES.length} transactions</p></div>
          <div className="kpi-card-neutral"><p className="text-xs text-muted-foreground mb-1">Panier moyen</p><p className="text-xl font-bold text-foreground tabular-nums">{Math.round(totalMois / VENTES.length).toLocaleString('fr-DZ')} DA</p></div>
              ...
              <tbody>
                {filtered.map((vente, idx) => (
                  <tr key={vente.id} className={`border-b border-border table-row-hover ${idx % 2 === 0 ? '' : 'bg-muted/20'}`}>
                    ...
                    <td className="px-5 py-3 text-right tabular-nums font-bold text-foreground">{vente.montant.toLocaleString('fr-DZ')} DA</td>
