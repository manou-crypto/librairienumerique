      ...
      <div className="px-6 py-6 max-w-2xl mx-auto space-y-6">
        ...
        <div className="card-base p-6 flex items-center gap-5">
          <div className="relative">
            ...
            <button className="absolute -bottom-1 -right-1 w-6 h-6 rounded-full bg-primary flex items-center justify-center shadow-sm hover:bg-blue-700 transition-colors">
          ...
          <div className="card-base p-6 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="sm:col-span-2">
                ...
                />
                <p className="text-xs text-muted-foreground mt-1">Le poste est géré par l'administrateur système</p>
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'securite' && (
          <div className="card-base p-6 space-y-4">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1.5">Mot de passe actuel</label>
                <input
                  type="password"
              ...
              <div>
                <label className="block text-xs font-semibold text-foreground mb-1.5">Confirmer le nouveau mot de passe</label>
                <input
                  type="password"
              ...
              {passwords.nouveau && passwords.confirmation && passwords.nouveau !== passwords.confirmation && (
            ...
            <div className="border-t border-border pt-4">
              ...
              <div className="space-y-2">
                {[
                  { device: 'Chrome — Windows 11', location: 'Alger, Algérie', current: true, time: 'Maintenant' },
                  { device: 'Firefox — Android', location: 'Alger, Algérie', current: false, time: 'Il y a 2h' },
                ].map((session, i) => (
                  <div key={i} className="flex items-center justify-between py-2.5 px-3 rounded-lg bg-muted/50">
                    ...
                    {session.current ? (
                      <span className="badge-active text-[10px]">Session actuelle</span>
                    ) : (
                      <button className="text-xs text-negative hover:underline">Déconnecter</button>
                    )}
                  </div>
                ))}
              </div>
          ...
          <div className="card-base p-6 space-y-5">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select value={prefs.theme} onChange={(e) => setPrefs({ ...prefs, theme: e.target.value })} className="input-field text-sm w-auto">
              ...
              <div>
                ...
                <select value={prefs.langue} onChange={(e) => setPrefs({ ...prefs, langue: e.target.value })} className="input-field text-sm w-auto">
                ...
                {[
                  ...
                  <div key={item.key} className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className={`relative w-11 h-6 rounded-full transition-colors ${prefs[item.key as keyof typeof prefs] ? 'bg-primary' : 'bg-muted'}`}
                    >
                      <span className={`absolute top-1 w-4 h-4 rounded-full bg-white shadow transition-transform ${prefs[item.key as keyof typeof prefs] ? 'translate-x-6' : 'translate-x-1'}`} />
                    </button>
                  </div>
                ))}
              </div>
        ...
        <div className="flex items-center justify-end gap-3">
          {saved && (
            <div className="flex items-center gap-1.5 text-sm text-positive fade-in">
              <CheckCircle size={15} /> Profil mis à jour
            </div>
          )}
