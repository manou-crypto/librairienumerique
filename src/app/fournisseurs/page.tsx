    ...
    <AppLayout currentPath="/fournisseurs">
      ...
      <div className="px-6 py-6 max-w-screen-2xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="kpi-card-info">
            ...
            <p className="text-xs text-muted-foreground mt-1">{FOURNISSEURS.filter(f => f.statut === 'actif').length} actifs</p>
          </div>
          <div className="kpi-card-positive">
            <p className="text-xs text-muted-foreground mb-1">Volume total achats</p>
            <p className="text-xl font-bold text-foreground tabular-nums">
              {FOURNISSEURS.reduce((s, f) => s + f.totalAchats, 0).toLocaleString('fr-DZ')} DA
            </p>
          </div>
          <div className="kpi-card-neutral">
            <p className="text-xs text-muted-foreground mb-1">Note moyenne</p>
            <p className="text-xl font-bold text-foreground tabular-nums">
              {(FOURNISSEURS.reduce((s, f) => s + f.note, 0) / FOURNISSEURS.length).toFixed(1)} / 5
            </p>
          </div>
          <div className="kpi-card-warning">
            <p className="text-xs text-muted-foreground mb-1">Inactifs</p>
            <p className="text-xl font-bold text-warning tabular-nums">{FOURNISSEURS.filter(f => f.statut === 'inactif').length}</p>
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/50">
                  ...
                  <th className="text-left px-5 py-3 text-xs font-semibold text-muted-foreground">Note</th>
                  <th className="text-right px-5 py-3 text-xs font-semibold text-muted-foreground">Total achats</th>
                  <th className="text-left px-5 py-3 text-xs font-semibold text-muted-foreground">Dernière commande</th>
                  ...
                  <tr key={f.id} className={`border-b border-border table-row-hover ${idx % 2 === 0 ? '' : 'bg-muted/20'}`}>
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold text-foreground text-sm">{f.nom}</p>
                    ...
                    <td className="px-5 py-3">
                      <p className="text-sm text-foreground">{f.contact}</p>
                      <div className="flex items-center gap-3 mt-0.5">
                        <a href={`tel:${f.telephone}`} className="text-xs text-muted-foreground flex items-center gap-1 hover:text-primary">
                    ...
                    <td className="px-5 py-3 text-right tabular-nums font-semibold text-foreground">
                      {f.totalAchats.toLocaleString('fr-DZ')} DA
                    </td>
                    <td className="px-5 py-3 text-muted-foreground">{f.derniereCommande}</td>
                    <td className="px-5 py-3 text-center">
                      <span className={f.statut === 'actif' ? 'badge-active' : 'badge-hidden'}>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-xl shadow-2xl w-full max-w-md fade-in">
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <h3 className="text-base font-bold text-foreground">{selected.nom}</h3>
            ...
            <div className="px-6 py-5 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm font-semibold text-foreground">{selected.categorie}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-0.5">Téléphone</p>
                  <p className="text-sm font-semibold text-foreground">{selected.telephone}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-0.5">Email</p>
                  <p className="text-sm font-semibold text-foreground text-xs">{selected.email}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground mb-0.5">Total achats</p>
                  <p className="text-sm font-bold text-primary">{selected.totalAchats.toLocaleString('fr-DZ')} DA</p>
                </div>
                <div>
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border">
              ...
              <button className="btn-primary text-sm py-2">Modifier</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-card rounded-xl shadow-2xl w-full max-w-md fade-in">
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <h3 className="text-base font-bold text-foreground">Nouveau fournisseur</h3>
            ...
            <div className="px-6 py-5 space-y-4">
              {[
                { label: 'Nom de la société', placeholder: 'Ex: Éditions Casbah', type: 'text' },
                { label: 'Nom du contact', placeholder: 'Prénom Nom', type: 'text' },
                { label: 'Email', placeholder: 'contact@societe.dz', type: 'email' },
                { label: 'Téléphone', placeholder: '+213 XX XX XX XX', type: 'tel' },
              ].map((field) => (
                <div key={field.label}>
                  <label className="block text-xs font-semibold text-foreground mb-1.5">{field.label}</label>
                  <input type={field.type} placeholder={field.placeholder} className="input-field text-sm" />
                </div>
              ))}
              <div>
            ...
            <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-border">
              ...
              <button onClick={() => setShowModal(false)} className="btn-primary text-sm py-2">Enregistrer</button>
            </div>
          </div>
        </div>
      )}
