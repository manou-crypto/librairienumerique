    ...
    <div className="flex-1 flex flex-col overflow-hidden">
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3 min-w-[180px]">
                      <div className="w-8 h-8 rounded-lg bg-muted flex items-center justify-center shrink-0 text-sm">
                        {product.categoryId === 'cat-livres' ? '📚' :
                         product.categoryId === 'cat-informatique' ? '💻' :
                         product.categoryId === 'cat-bureautique' ? '🗂️' : '✏️'}
                      </div>
                  ...
                  <td className="px-4 py-3 text-right">
                    <span className="text-xs tabular-nums text-muted-foreground">
                      {product.prixAchat.toLocaleString('fr-FR', { minimumFractionDigits: 2 })} €
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right">
                  ...
                  <td className="px-4 py-3 text-right">
                    {getStockDisplay(product.stock, product.seuilAlerte)}
                  </td>
                  ...
                  <td className="px-4 py-3 text-center">
                    <button
                      ...
                      className={`p-1.5 rounded-lg transition-colors ${
                        product.visible
                          ? 'text-green-600 hover:bg-green-50' :'text-muted-foreground hover:bg-muted'
                      }`}
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      ...
                      <button
                        ...
                        title={`Supprimer ${product.name} — cette action est irréversible`}
                        aria-label={`Supprimer ${product.name}`}
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
                    {/* Always show on non-group-hover capable devices */}
      ...
      <div className="border-t border-border px-5 py-3 bg-card flex items-center justify-between gap-4 shrink-0 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          ...
          <select className="text-xs border border-border rounded-lg px-2 py-1 bg-card focus:outline-none focus:ring-1 focus:ring-ring">
            <option>10</option>
            <option>25</option>
            <option>50</option>
          </select>
