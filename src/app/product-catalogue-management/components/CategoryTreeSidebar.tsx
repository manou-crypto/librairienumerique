...
const categoryTree: Category[] = [
  {
    ...
    icon: '📚',
    children: [
      { id: 'cat-romans', label: 'Romans & Littérature', icon: '📖' },
      { id: 'cat-scolaires', label: 'Manuels scolaires',icon: '📝' },
      { id: 'cat-dictionnaires', label: 'Dictionnaires', icon: '📗' },
    ],
  },
  {
    id: 'cat-fournitures',
    ...
    icon: '✏️',
    children: [
      { id: 'cat-cahiers', label: 'Cahiers & Carnets', icon: '📓' },
      { id: 'cat-stylos', label: 'Stylos & Crayons', icon: '🖊️' },
      { id: 'cat-colle', label: 'Colle & Ciseaux', icon: '✂️' },
    ],
  },
  {
    id: 'cat-informatique',
    ...
    icon: '💻',
    children: [
      { id: 'cat-peripheriques', label: 'Périphériques', icon: '🖱️' },
      { id: 'cat-consommables', label: 'Consommables', icon: '🖨️' },
      { id: 'cat-stockage', label: 'Stockage', icon: '💾' },
    ],
  },
  {
    id: 'cat-bureautique',
    ...
    icon: '🗂️',
    children: [
      { id: 'cat-classement', label: 'Classement', icon: '📁' },
      { id: 'cat-papier', label: 'Papier & Impression', icon: '📄' },
    ],
  },
];
...
function countByCategory(products: Product[], catId: string): number {
    ...
    <div>
      <button
        ...
        className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all duration-150 ${
          isSelected ? 'nav-item-active' : 'nav-item-inactive'
        }`}
        style={{ paddingLeft: `${12 + depth * 14}px` }}
      >
        {hasChildren ? (
          <span className="text-muted-foreground w-4 shrink-0" onClick={(e) => { e.stopPropagation(); setExpanded(!expanded); }}>
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin p-2">
        ...
        <button
          ...
          className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-all duration-150 mb-1 ${
            selectedCategory === 'all' ? 'nav-item-active' : 'nav-item-inactive'
          }`}
        >
