...
const ROLE_REDIRECTS: Record<Role, string> = {
  super_admin: '/',
  manager: '/product-catalogue-management',
...
export default function LoginPage() {
  ...
  const fillDemo = (user: UserCredentials) => {
    setEmail(user.email);
              ...
              {DEMO_USERS.map((user) => (
                ...
                >
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold ${
                    ...
                    user.role === 'manager'? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'
                  }`}>
                    {user.name.split(' ').map(n => n[0]).join('')}
                  </div>
                  <span className="text-[10px] font-semibold text-foreground leading-tight">{ROLE_LABELS[user.role]}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="mt-4 card-base p-4">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className="font-semibold text-foreground">Gestionnaire</span>
              <span className="text-muted-foreground">— Catalogue, stocks, achats, rapports</span>
            </div>
            <div className="flex items-center gap-2 text-xs">
              <span className="w-2 h-2 rounded-full bg-amber-500 shrink-0" />
              <span className="font-semibold text-foreground">Caissier</span>
              <span className="text-muted-foreground">— Point de vente uniquement</span>
            </div>
          </div>
        </div>
