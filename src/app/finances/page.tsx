...
const CA_DATA = [
  { mois: 'Mar', ca: 285000, benefice: 82000 },
  { mois: 'Avr', ca: 312000, benefice: 95000 },
  { mois: 'Mai', ca: 298000, benefice: 88000 },
  { mois: 'Juin', ca: 345000, benefice: 112000 },
  { mois: 'Juil', ca: 389000, benefice: 128000 },
  { mois: 'Août', ca: 156000, benefice: 52000 },
];

const CATEGORIES_DATA = [
  { name: 'Livres', value: 38 },
  { name: 'Informatique', value: 32 },
  { name: 'Fournitures', value: 18 },
      ...
      <div className="px-6 py-6 max-w-screen-2xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="kpi-card-info">
            ...
            <p className="text-xl font-bold text-foreground tabular-nums">{totalCA.toLocaleString('fr-DZ')} DA</p>
            <div className="flex items-center gap-1 mt-1">
              <TrendingUp size={12} className="text-positive" />
              <p className="text-xs text-positive">+12.4% vs période préc.</p>
            </div>
          </div>
          <div className="kpi-card-positive">
            ...
            <p className="text-xl font-bold text-foreground tabular-nums">{totalBenefice.toLocaleString('fr-DZ')} DA</p>
            <div className="flex items-center gap-1 mt-1">
              <TrendingUp size={12} className="text-positive" />
              <p className="text-xs text-positive">+8.7% vs période préc.</p>
            </div>
          </div>
          ...
          <div className="kpi-card-warning">
            ...
            <p className="text-xl font-bold text-foreground tabular-nums">83 900 DA</p>
            <div className="flex items-center gap-1 mt-1">
              <TrendingDown size={12} className="text-negative" />
              <p className="text-xs text-negative">−10.2% vs hier</p>
            </div>
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-muted/50">
                  ...
                  <th className="text-right px-5 py-3 text-xs font-semibold text-muted-foreground">Caisse 1</th>
                  <th className="text-right px-5 py-3 text-xs font-semibold text-muted-foreground">Caisse 2</th>
                ...
                {CLOTURES.map((c, idx) => (
                  <tr key={c.date} className={`border-b border-border table-row-hover ${idx % 2 === 0 ? '' : 'bg-muted/20'}`}>
                    ...
                    <td className="px-5 py-3 text-center">
                      <span className={c.statut === 'en_cours' ? 'badge-draft' : 'badge-active'}>
